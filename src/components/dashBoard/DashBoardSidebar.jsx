...
export default function DashboardSidebar({ role = "owner" }) {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden w-72 shrink-0 md:block h-screen z-[60] relative sticky top-0">
          {navContent}
      </aside>
      ...
      <div className={`fixed top-0 left-0 h-full w-72 bg-surface-container z-[120] transform transition-transform duration-300 ease-in-out md:hidden ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <button 
          onClick={() => setIsOpen(false)}
